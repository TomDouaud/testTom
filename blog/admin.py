...
from .models import AlbumOfTheMonth, AlbumTrack, AlbumAtHomeImage, Playlist, Track, MovieImage, PasswordResetRequest

class AlbumTrackInline(admin.TabularInline):
    model = AlbumTrack
    extra = 1

class AlbumAtHomeImageInline(admin.TabularInline):
    model = AlbumAtHomeImage
    extra = 1
...
class AlbumOfTheMonthAdmin(admin.ModelAdmin):
    ...
    list_filter = ('month',)
    inlines = [AlbumTrackInline, AlbumAtHomeImageInline]
